...
, final as (
    select
        ...
        , coalesce(zendesk_ticket_calls_summary.ticket_created_at_est, zendesk_ticket_calls_summary.call_created_at_est) as dispute_timestamp
    ...
    where zendesk_ticket_calls_summary.brand_id in (select brand_id from fbt_brands)
        and zendesk_ticket_calls_summary.ticket_group = 'Disputes'
...
, stakeholder_review as (
    ...
    where report_table_name = 'fbt_universal_report_billing_payment_disputes'
...
select
    final.*
    , 'fbt_universal_report_billing_payment_disputes' as table_name
